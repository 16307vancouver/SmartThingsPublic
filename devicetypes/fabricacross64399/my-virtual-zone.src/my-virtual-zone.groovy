...
metadata {
	...
	definition (name: "My Virtual Zone", namespace: "fabricacross64399", author: "Yves Racine",mnmn: "SmartThingsCommunity", vid: "25256a45-ffa7-32b4-a161-06e4888cd643", ocfDeviceType:"oic.r.temperature") {
		//vid: "25256a45-ffa7-32b4-a161-06e4888cd643",60ed93bc-e988-36b2-9667-35e8efa581c6
	...
	String tempValueString    
 	if (scale== 'C') { 	
		...
		if (tempValueString.matches(".*([.,][3456])")) {                
			...
			traceEvent(settings.logFilter,"setThermostatSetpoint>curSetpoint's value ($tempValueString) which ends with 3456=> rounded to .5", settings.trace,get_LOG_INFO())	
		} else if (tempValueString.matches(".*([.,][789])")) {  
			traceEvent(settings.logFilter,"setThermostatSetpoint>curSetpoint's value ($tempValueString) which ends with 789=> rounded to next .0", settings.trace,get_LOG_INFO())	
			tempValue=tempValue.intValue() + 1                        
			tempValueString=String.format('%2d.0', tempValue.intValue())               
		} else {
			traceEvent(settings.logFilter,"setThermostatSetpoint>curSetpoint's value ($tempValueString) which ends with 012=> rounded to previous .0", settings.trace,get_LOG_INFO())	
			tempValueString=String.format('%2d.0', tempValue.intValue())               
		}
    
		float newTempDelta= (tempValue  - mainSetpoint.toFloat()).round(1)      
		traceEvent(settings.logFilter,"setThermostatSetpoint in Celsius>after temp correction= $temp, tempDelta=$newTempDelta",settings.trace)
		if ((newTempDelta.toDouble() < tempValue) && (curTempDelta.toFloat() != newTempDelta) && (newTempDelta < MAX_DELTA)) {        
			...
			traceEvent(settings.logFilter,"setThermostatSetpoint in Celsius>new thermostatSetpoint: $tempValueString, new tempDelta=$tempDelta",settings.trace)
		}  else {
			traceEvent(settings.logFilter,"setThermostatSetpoint>max delta reached ($MAX_DELTA)..", settings.trace,get_LOG_INFO())	
			sendEvent(name:"tempDelta", value: MAX_DELTA, displayed: true, unit:scale)
			sendEvent(name:"thermostatSetpoint", value: tempValueString, displayed: true, unit:scale)
			traceEvent(settings.logFilter,"setThermostatSetpoint in Celsius>new thermostatSetpoint: $tempValueString, new tempDelta=$MAX_DELTA",settings.trace)
		}        
			        
	} else {
		double tempValue=value
		double newTempDelta= (tempValue  - mainSetpoint.toDouble()) 
		traceEvent(settings.logFilter,"setThermostatSetpoint in Fahrenheit>after temp correction= $temp, tempDelta=$newTempDelta",settings.trace)
		if (newTempDelta.toDouble() < tempValue && curTempDelta != newTempDelta && (newTempDelta < MAX_DELTA)) {        
			...
			sendEvent(name:"thermostatSetpoint", value: tempValueString, displayed: true, unit:scale)
			traceEvent(settings.logFilter,"setThermostatSetpoint in Fahrenheit>new thermostatSetpoint: $tempValueString, new tempDelta=$tempDelta",settings.trace)
		}  else {
			traceEvent(settings.logFilter,"setThermostatSetpoint>max delta reached ($MAX_DELTA)..", settings.trace,get_LOG_INFO())	
			sendEvent(name:"tempDelta", value: MAX_DELTA, displayed: true, unit:scale)
			sendEvent(name:"thermostatSetpoint", value: tempValueString, displayed: true, unit:scale)
			traceEvent(settings.logFilter,"setThermostatSetpoint in Fahrenheit>new thermostatSetpoint: $tempValueString, new tempDelta=$MAX_DELTA",settings.trace)
		}        
